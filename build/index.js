...
import { ContentTypes, FeedListingTypes, FeedTypes } from "graphlit-client/dist/generated/graphql-types.js";
dotenv.config({ path: '.env.local' });
const server = new McpServer({
    name: "Graphlit MCP Server",
    version: "1.0.0"
});
function formatResultsToXML(results) {
    const xmlParts = [];
...
server.tool("retrieveSlackMessages", `Retrieve Slack messages from Graphlit knowledge base.
   ...
   Prompt should be optimized for vector search, via text embeddings. Rewrite prompt as appropriate for higher relevance to search results.`, {
    ...
    try {
        ...
        const filter = { types: [ContentTypes.Message] };
...
server.tool("ingestSlack", `Ingests messages from Slack channel into Graphlit knowledge base.
